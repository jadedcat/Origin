...
public class DepLoader implements IFMLLoadingPlugin, IFMLCallHook {
	...
	public static class Downloader extends JOptionPane implements IDownloadDisplay {
		...
		private Box makeProgressPanel() {
			...
			welcomeLabel = new JLabel("<html>Please wait, " + owner
					+ " has some tasks to do before you can play</html>");
			welcomeLabel.setAlignmentY(LEFT_ALIGNMENT);
			box.add(welcomeLabel);
			box.add(Box.createRigidArea(new Dimension(0, 10)));
		...
		public JDialog makeDialog() {
			...
			addPropertyChangeListener(new PropertyChangeListener() {
				...
				public void propertyChange(PropertyChangeEvent evt) {
					if (evt.getSource() == Downloader.this
							&& evt.getPropertyName() == VALUE_PROPERTY) {
						requestClose(
								"This will stop minecraft from launching\nAre you sure you want to do this?");
					}
				}
			});
		...
		protected void requestClose(String message) {
			int shouldClose = JOptionPane
					.showConfirmDialog(container, message, "Are you sure you want to stop?",
		...
		public void showErrorDialog(String name, String url) {
			...
			JOptionPane.showMessageDialog(null, ep, "A download error has occured",
	...
	public static class DepLoadInst {
		...
		private void deleteMod(File mod) {
			...
			try {
				...
				Closeable loader = ((Map<String, Closeable>) f_lmap.get(ucp))
						.remove(URLUtil.urlNoFragString(url));
				if (loader != null) {
					loader.close();
				...
				System.err.println(
						"This error is most likely caused by an invalid java install. Please reinstall java.");
				e.printStackTrace();
			}
			...
			if (!mod.delete()) {
				...
				if (!GraphicsEnvironment.isHeadless())
					JOptionPane.showMessageDialog(null, msg, "An update error has occured",
							JOptionPane.ERROR_MESSAGE);
		...
		private void download(Dependency dep) {
			...
			try {
				...
				downloadMonitor
						.showErrorDialog(dep.file.filename, dep.url + '/' + dep.file.filename);
		...
		private void download(InputStream is, int sizeGuess, File target) throws Exception {
			if (sizeGuess > downloadBuffer.capacity())
				throw new Exception(String.format(
						"The file %s is too large to be downloaded by " + owner
								+ " - the download is invalid", target.getName()));
		...
		private void loadDeps() {
			downloadMonitor = FMLLaunchHandler.side().isClient() ?
					new Downloader() :
					new DummyDownloader();
			try {
		...
		private boolean mergeNew(Dependency oldDep, Dependency newDep) {
			...
			Dependency newest =
					newDep.file.version.compareTo(oldDep.file.version) > 0 ? newDep : oldDep;
