...
public class ContainerBase extends Container {
	...
	public boolean isAttachedToItem() {
		return this.inventory instanceof InventoryBase
				&& ((InventoryBase) this.inventory).isItemInventory();
	}
	 ...
	 * @return
	 */
	public InventoryBase getItemInventory() {
		if (this.isAttachedToItem()) return (InventoryBase) this.inventory;
		return null;
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slotiD) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (slotiD < inventorySize) {
				...
				if (targetSlotID >= 0) {
					for (int slotID = targetSlotID; slotID < this
							.getExcludedMaximumSlotIDForItemStack(stackInSlotCopy); slotID++) {
						Slot targetSlot = (Slot) this.inventorySlots.get(slotID);
						if (this.isItemValidForSlotOnShift(targetSlot, stackInSlotCopy)) {
							if (!this.mergeItemStack(stackInSlotCopy, slotID, slotID + 1, false))
								return null;
						}
					}
	...
	protected boolean isItemValidForSlotOnShift(Slot slot, ItemStack stackToProcess) {
		return slot instanceof GhostSlot ? false : slot.isItemValid(stackToProcess);
	}
	...
	public void writeToNBT() {
		if (this.isAttachedToItem()) {
			...
			if (heldStack != null) {
				...
				((InventoryBase) this.inventory).writeToNBT(invTagCom);
